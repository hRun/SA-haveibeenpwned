...
class hibpCommand(StreamingCommand):
    ...
    def stream(self, events):
        ...
        # Set up logging
        logger  = logging.getLogger('hibpCommand')
        ...
        '''if self.use_proxies == '1':
            self.proxies['http']  = settings.get('http_proxy')
            self.proxies['https'] = settings.get('https_proxy')
        else:
            self.proxies['http']  = ''
            self.proxies['https'] = '' '''
        ...
        for event in events:
            if self.mode == "domain":
                ...
                if data != 0:
                    ...
                    if len(breach) == 0:
                        event['breach'] = "No breach reported for given time frame."
                    else:
                        event['breach'] = ""
                        for entry in breach:
                            for item in entry:
                ...
                url       = 'https://haveibeenpwned.com/api/v3/breachedaccount/{0}'.format(event[self.fieldnames[0]])
                breach    = []
                ...
                url      = 'https://haveibeenpwned.com/api/v2/pasteaccount/{0}'.format(event[self.fieldnames[0]])
                paste    = []
                ...
                if response.status_code == 200:
                    ...
                    if len(paste) == 0:
                        event['paste'] = "No paste reported for given time frame."
                    else:
                        event['paste'] = ""
                        for entry in paste:
                            for item in entry:
