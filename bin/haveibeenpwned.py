...
class hibpCommand(StreamingCommand):
    ...
    def stream(self, events):
        ...
        if self.mode not in ['domain', 'mail']:
            raise RuntimeWarning(f'Invalid value for option "mode" specified: "{self.mode}"')
        if self.mode == 'mail' and self.pastes not in ['all', 'dated', 'none']:
            raise RuntimeWarning(f'Invalid value for option "pastes" specified: "{self.pastes}"')
        if self.output not in ['text', 'json']:
            raise RuntimeWarning(f'Invalid value for option "output" specified: "{self.output}"')

        try:
            int(self.threshold)
        except Exception:
            raise RuntimeWarning(f'Invalid value for option "threshold" specified: "{self.threshold}"')
        ...
        if api_key is not None and len(api_key) >= 32:
            headers = {'User-Agent': 'splunk-app-for-hibp/2.3.4', 'hibp-api-key': f'{api_key}'}
        else:
            headers = {'User-Agent': 'splunk-app-for-hibp/2.3.4'}
        ...
        if use_proxy == 1:
            ...
            if proxy_password:
                ...
                if self.py3:
                    auth_headers = {'Proxy-Authorization': f'Basic {base64.b64encode(auth.encode("utf-8")).decode("utf-8")}'}
                    # headers['Proxy-Authorization'] = f'Basic {base64.b64encode(auth.encode("utf-8")).decode("utf-8")}'
                else:
                    auth_headers = {'Proxy-Authorization': f'Basic {base64.b64encode(auth)}'}
            ...
            if proxy_type == "http":
                try:
                    connection = http_client.HTTPSConnection(f'{proxy_url.split("//")[-1].rstrip("/")}', port=proxy_port)
                    ...
                    connection.close()
                    logger.error(f"HTTPS proxy connection failed, falling back to HTTP proxy: {e1}")
                    try:
                        connection = http_client.HTTPConnection(f'{proxy_url.split("//")[-1].rstrip("/")}', port=proxy_port)
                        connection.set_tunnel('haveibeenpwned.com', port=443, headers=auth_headers)
                        connection.request('HEAD', '/api/v3', headers=headers)
                        r = connection.getresponse()
                        r.read()
                        ...
                        connection.close()
                        logger.error(f"HTTP proxy connection failed: {e2}")
                        raise RuntimeWarning(f"Proxy connection attempts failed, please check your configuration and connectivity: {e1}, {e2}")
            """if proxy_type == "socks4":
                try:
                    connection = http_client.HTTPSConnection(f'{proxy_url.split("//")[-1].rstrip("/")}', port=proxy_port)
                    connection.set_tunnel('haveibeenpwned.com', port=443, headers=auth_headers)
                    connection.sock = socks.socksocket()
                    connection.sock.set_proxy(socks.PROXY_TYPE_SOCKS4, f'{proxy_url.split("//")[-1].rstrip("/")}', proxy_port)
                    connection.sock.connect(('haveibeenpwned.com', 443))
                except Exception as e:
                    connection.close()
                    raise RuntimeWarning(f"Proxy connection attempts failed, please check your configuration and connectivity: {e}")
            if proxy_type == "socks5":
                try:
                    connection = http_client.HTTPSConnection(f'{proxy_url.split("//")[-1].rstrip("/")}', port=proxy_port)
                    connection.set_tunnel('haveibeenpwned.com', port=443, headers=auth_headers)
                    connection.sock = socks.socksocket()
                    connection.sock.set_proxy(socks.PROXY_TYPE_SOCKS5, f'{proxy_url.split("//")[-1].rstrip("/")}', proxy_port)
                    connection.sock.connect(('haveibeenpwned.com', 443))
                except Exception as e:
                    connection.close()
                    raise RuntimeWarning(f"Proxy connection attempts failed, please check your configuration and connectivity: {e}")"""
        ...
        for event in events:
            ...
            if self.mode == "domain":
                ...
                if tracker == 0:
                    try:
                        ...
                        connection.close()
                        logger.error(f"HTTPS request failed: {e}")
                        raise RuntimeWarning(f"HTTPS request failed: {e}")
                ...
                if data is not None:
                    for entry in json.loads(data.decode('utf8')):
                        if int((date - datetime.datetime.strptime(entry['AddedDate'], '%Y-%m-%dT%H:%M:%SZ')).days) > int(self.threshold) \
                            or not (event[self.fieldnames[0]] == entry['Domain'] or event[self.fieldnames[0]] == f".{entry['Domain']}"):
                            pass
                        else:
                            dataclass = []
                            ...
                            if self.output == "text":
                                breach.append(
                                    [
                                        ...
                                        f"Date of Breach: {entry['BreachDate']}",
                                        f"Date of Availability: {entry['AddedDate']}",
                                        f"Breached Accounts: {entry['PwnCount']}",
                                        f"Breached Data: {', '.join(dataclass)}",
                                        f"Breach Description: {entry['Description']}"
                                    ]
                                )
                            else:
                                breach.append(
                                    {
                                        "Title": f"{entry['Title']}",
                                        "Domain": f"{entry['Domain']}",
                                        "Date of Breach": f"{entry['BreachDate']}",
                                        "Date of Availability": f"{entry['AddedDate']}",
                                        "Breached Accounts": f"{entry['PwnCount']}",
                                        "Breached Data": f"{', '.join(dataclass)}",
                                        "Breach Description": f"{entry['Description']}"
                                    }
                                )
                    ...
                    if len(breach) == 0:
                        ...
                        if self.output == "text":
                            ...
                            for i, entry in enumerate(breach):
                                event['breach'] += f'"Breach {i+1}": {entry}, '
                            # event['breach'].rstrip(', ')
                            event['breach'] += "}"
            ...
            # Check for account breaches and pastes
                ...
                try:
                    if not use_proxy == 1:
                        connection = http_client.HTTPSConnection('haveibeenpwned.com', 443)
                    connection.request('GET', f'/api/v3/breachedaccount/{url_parse.quote_plus(event[self.fieldnames[0]])}?truncateResponse=false', headers=headers)
                    response = connection.getresponse()
                except Exception as e:
                    connection.close()
                    logger.error(f"HTTPS request failed: {e}")
                ...
                if response.status == 200:
                    ...
                    if self.output == "text":
                        event['breach'] = f"Received {response.status} HTTP response code from API, no breach could be retrieved."
                    else:
                        event['breach'] = {"Message": f"Received {response.status} HTTP response code from API, no breach could be retrieved."}
                ...
                if self.pastes in ['all', 'dated']:
                    try:
                        if not use_proxy == 1:
                            connection = http_client.HTTPSConnection('haveibeenpwned.com', 443)
                        connection.request('GET', f'/api/v3/pasteaccount/{url_parse.quote_plus(event[self.fieldnames[0]])}', headers=headers)
                        response = connection.getresponse()
                    except Exception as e:
                        connection.close()
                        logger.error(f"HTTPS request failed: {e}")
                    ...
                    if response.status == 200:
                        ...
                        if self.output == "text":
                            event['paste'] = f"Received {response.status} HTTP response code from API, no paste could be retrieved."
                        else:
                            event['paste'] = {"Message": f"Received {response.status} HTTP response code from API, no paste could be retrieved."}
